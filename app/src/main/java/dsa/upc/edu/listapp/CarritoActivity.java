...
//        });
        ...
        api = ApiClient.getClient(CarritoActivity.this).create(ApiService.class);

        // Adapter con listener de eliminar
        adapter = new CarritoAdapter();
        adapter.setOnRemoveClickListener((producto, pos) -> eliminarDelCarrito(producto.getId_objeto(), pos));
    ...
    private void cargarCarrito() {
        ...
        api.getObjetosDelCarrito(idPartida).enqueue(new Callback<List<Objeto>>() {
            @Override
            public void onResponse(Call<List<Objeto>> call, Response<List<Objeto>> resp) {
                swipe.setRefreshing(false);
                if (resp.isSuccessful() && resp.body() != null) {
                    List<Objeto> lista = resp.body();
    ...
    private void actualizarTotal(List<Objeto> lista) {
        double total = 0;
        for (Objeto producto : lista) total += producto.getPrecio();
        tvTotal.setText(String.format("Total: %.2f€", total));
    }

    private void eliminarDelCarrito(String idProd, int pos) {
        api.eliminarObjetoDelCarrito(idPartida, idProd).enqueue(new Callback<Void>() {
            @Override
    ...
    private void vaciarCarrito() {
        api.vaciarCarrito(idPartida).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> resp) {
                if (resp.isSuccessful()) {
                    adapter.clear();
    ...
    private void actualizarMonedas() {
        api.getMonedasDePartida(idPartida).enqueue(new Callback<MonedasResponse>() {
            @Override
