...
public class RandomActivity extends AppCompatActivity {
    ...
    private void cargarProductoAleatorio() {
        api = ApiClient.getClient(RandomActivity.this).create(ApiService.class);
        api.getProductoAleatorio().enqueue(new Callback<Objeto>() {
            ...
            public void onResponse(Call<Objeto> call, Response<Objeto> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    tvCategoria.setText("Categoría: " + productoMostrado.getCategoria().getNombre());
                    ...
                    // Configurar compra
                    btnComprarAleatorio.setOnClickListener(v -> realizarCompraAleatoria(productoMostrado.getId_objeto()));
                } else {
    ...
    private void realizarCompraAleatoria(String idObjeto) {
        api.comprarObjetoAleatorio(idPartida, idObjeto).enqueue(new Callback<Void>() {
            @Override
